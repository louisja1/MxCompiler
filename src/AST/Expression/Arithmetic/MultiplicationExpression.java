...
public class MultiplicationExpression extends BaseExpression {
    ...
    private MultiplicationExpression(BaseExpression _firstExpression, BaseExpression _secondExpression) {
    ...
    public static BaseExpression getExpression(BaseExpression _firstExpression, BaseExpression _secondExpression) {
        ...
        if (firstType == IntType.getInstance() && secondType == IntType.getInstance()) {
            ...
            return new MultiplicationExpression(_firstExpression, _secondExpression);
        }
        throw new CompilationError("There is an invalid multiplication expression");
    }
    ...
    public void generateIR(List<BaseInstruction> instructionList) {
        ...
        if (first instanceof Immediate && ((Immediate) first).value == 1) {
        ...
        if (flag) {
            ...
            for (int i = 1; i <= 6; i++) {
                if (((Immediate) second).value == (1 << i)) {
                    instructionList.add(new BinaryInstruction(Operator.BinaryOp.SHL, result, new Immediate(i)));
                    flag = true;
                    break;
                }
            }
        ...
        if (!flag) {
            instructionList.add(new BinaryInstruction(Operator.BinaryOp.MUL, result, second));
        }
