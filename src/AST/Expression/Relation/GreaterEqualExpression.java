...
public class GreaterEqualExpression extends BaseExpression {
    private BaseExpression firstExpression, secondExpression;

    private GreaterEqualExpression(BaseExpression _firstExpression, BaseExpression _secondExpression) {
    ...
    public static BaseExpression getExpression(BaseExpression _firstExpression, BaseExpression _secondExpression) {
        ...
        if (firstType == IntType.getInstance() && secondType == IntType.getInstance()) {
            ...
            return new GreaterEqualExpression(_firstExpression, _secondExpression);
        }
        if (firstType == StringType.getInstance() && secondType == StringType.getInstance()) {
            ...
            return FunctionExpression.getExpression(IdentifierExpression.getExpression("_builtin_GREQ"), expressionList);
        }
        throw new CompilationError("There is an invalid greater equal expression");
    }
    ...
    public void generateIR(List<BaseInstruction> instructionList) {
        ...
        instructionList.add(new CsetInstruction(Operator.ConditionOp.GREQ, result));
